...
import io.github.gaomingyuan666.base.exception.ErrorCode;
...
public class AssertUtil {
    ...
    public static <T> void fastFailValidate(T obj) {
        ...
        if (constraintViolations.size() > 0) {
            throwException(BizErrorCode.PARAM_VALID, constraintViolations.iterator().next().getMessage());
        }
    }
    ...
    public static <T> void allCheckValidateThrow(T obj) {
        ...
        if (constraintViolations.size() > 0) {
            ...
            //去掉最后一个逗号
            throwException(BizErrorCode.PARAM_VALID, errorMsg.toString().substring(0, errorMsg.length() - 1));
        }
    }
    ...
    private static void throwException(ErrorCode errorEnum, Object... arg) {
        if (Objects.isNull(errorEnum)) {
            errorEnum = BizErrorCode.BIZ_ERROR;
        }
        throw new BizException(MessageFormat.format(errorEnum.getMessage(), arg));
    }
