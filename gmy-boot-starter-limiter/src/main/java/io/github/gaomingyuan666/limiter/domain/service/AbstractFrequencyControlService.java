...
import io.github.gaomingyuan666.base.utils.AssertUtil;
 ...
 *
 * @author gaomingyuan
 * @see FixedWindowRateLimiterController 通用限流类
 */
@Slf4j
public abstract class AbstractFrequencyControlService<K extends FrequencyControlDTO> {
    ...
    protected void registerMyselfToFactory() {
        FrequencyControlStrategyFactory.registerFrequencyController(getStrategyName().name(), this);
    }
    ...
    private <T> T executeWithFrequencyControlMap(Map<String, K> frequencyControlMap, SupplierThrowWithoutParam<T> supplier) throws Throwable {
        if (reachRateLimit(frequencyControlMap)) {
            throw new BizException(BizErrorCode.FREQUENCY_LIMIT);
        }
        try {
            return supplier.get();
        } finally {
     ...
     * @return 策略名称
     */
    protected abstract LimiterTypeEnum getStrategyName();
}
